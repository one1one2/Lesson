...
// Transfinite Line 这个命令能够显式的指定点数和他们的分布。
// 'Progression 2' 意思是说这个系列中的所有线段会是前一个的
// 两倍。
...
// 所有的二维和三维的 Transfinite 实体都是通过点的 Transfinite
// 来定义的。其中点的顺序表示了网格中元素的顺序。
...
// 和做曲面推移的时候一样，重组命令 Recombine 告诉 Gmsh
// 在可能的情况下将单纯形重新组合成为四边形、三棱柱或者
// 六面体。下面的列表中的分号的效果就像一个循环一样（和
// Matlab 中的用法相似）：所有编号 55 到 127 之间的曲面
// 都在考虑之列。
